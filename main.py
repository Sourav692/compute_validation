...
from compute_validator.runner import run


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Databricks compute (clusters, SQL warehouses, instance pools) against YAML expectations."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--profile",
        default=None,
        help="Databricks CLI profile to use (defaults to SDK auth resolution: env vars / DEFAULT profile).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
